...
export default function CountriesPage() {
  ...
  const loadCountries = async () => {
    setIsLoading(true);
    const items = await Country.list();
    setCountries(items);
    setIsLoading(false);
  };
  ...
  const handleEdit = (country) => {
    ...
    setFormData({
      ...
      code: country.code || '',
      integration_key: country.integration_key || '',
      is_active: country.is_active !== undefined ? country.is_active : true,
      from_date: country.from_date || '',
      to_date: country.to_date || ''
    });
  ...
  const handleToggleActive = async (country) => {
    try {
      await Country.update(country.id, { ...country, is_active: !country.is_active });
      loadCountries();
    } catch (error) {
  ...
  const handleSave = async () => {
    ...
    if (editingCountry) {
      await Country.update(editingCountry.id, dataToSend);
    } else {
      await Country.create(dataToSend);
    }
    loadCountries();
  ...
  return (
    <div className="p-4 space-y-4">
      ...
      <Card className="clay-card">
        <CardHeader>
          <CardTitle className="flex items-center gap-2"><Globe className="w-5 h-5 text-emerald-600" /> All Countries</CardTitle>
        </CardHeader>
        <CardContent>
          {isLoading ? (
              ...
              <TableBody>
                {countries.map(country => (
                  <TableRow key={country.id}>
                    ...
                    <TableCell className="font-arabic">{country.arabic_name || '-'}</TableCell>
                    <TableCell><span className="font-mono text-sm bg-gray-100 px-2 py-1 rounded">{country.code}</span></TableCell>
                    <TableCell className="text-sm text-gray-600">{country.integration_key || '-'}</TableCell>
                    <TableCell>
                      <Badge className={`${country.is_active ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'}`}>
                        {country.is_active ? 'Active' : 'Inactive'}
                      </Badge>
                    </TableCell>
                    <TableCell className="text-sm">{country.from_date || '-'}</TableCell>
                    <TableCell className="text-sm">{country.to_date || '-'}</TableCell>
                    <TableCell className="text-right">
                      <Button variant="ghost" size="icon" onClick={() => handleToggleActive(country)} title={country.is_active ? "Deactivate" : "Activate"}>
                        {country.is_active ? <PowerOff className="w-4 h-4 text-orange-500" /> : <Power className="w-4 h-4 text-green-500" />}
                      </Button>
                      <Button variant="ghost" size="icon" onClick={() => handleEdit(country)}>
                        <Edit className="w-4 h-4" />
                      </Button>
                      <Button variant="ghost" size="icon" onClick={() => handleDelete(country.id)} className="text-red-500">
        ...
        <DialogContent className="clay-card sm:max-w-3xl">
          ...
          <div className="py-4 space-y-6">
            ...
            <div className="flex items-center space-x-2 pt-4 border-t">
              ...
              />
              <Label htmlFor="is_active">Country is Active</Label>
            </div>
          </div>
          <DialogFooter>
